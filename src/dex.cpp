...
mars_status_t dex::schema(sqlite3 *db)
{
    ...
    static const char *view_sql =
        ...
        "DROP VIEW IF EXISTS dex_trade_bars;"
        "CREATE VIEW dex_trade_bars AS "
        "WITH agg AS ("
        " SELECT (s.ts / 60) * 60 AS ts,"
        "  count(*) AS swap_count,"
        "  sum(CASE WHEN substr(s.amount1_raw,1,1)='-' "
        "   OR lower(substr(s.amount1_raw,3,1)) >= '8' "
        "   THEN s.base_amount ELSE 0.0 END) AS buy_base_amount,"
        "  sum(CASE WHEN substr(s.amount1_raw,1,1)!='-' "
        "   AND lower(substr(s.amount1_raw,3,1)) < '8' "
        "   THEN s.base_amount ELSE 0.0 END) AS sell_base_amount,"
        ...
        " c.price AS price,"
        " a.buy_base_amount AS buy_base_amount,"
        " a.sell_base_amount AS sell_base_amount,"
        " a.quote_amount AS quote_amount,"
